...
namespace rent_a_car.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                // Uniqueness validations: EGN, Email, PhoneNumber
                // Check EGN uniqueness
                if (!string.IsNullOrWhiteSpace(Input.EGN))
                {
                    var egnExists = await _userManager.Users.AnyAsync(u => u.EGN == Input.EGN);
                    if (egnExists)
                    {
                ...
                // If any uniqueness checks failed, redisplay form
                if (!ModelState.IsValid)
                {
                    return Page();
                }
                ...
                // Assign custom properties BEFORE creating the user so non-nullable DB columns are populated.
                user.FirstName = Input.FirstName;
                user.LastName = Input.LastName;
                user.EGN = Input.EGN;
                user.PhoneNumber = Input.PhoneNumber;

                // Use the provided username and email
                ...
                if (result.Succeeded)
                {
                    ...
                    _logger.LogInformation("Role 'User' assigned to user {UserId}", user.Id);
